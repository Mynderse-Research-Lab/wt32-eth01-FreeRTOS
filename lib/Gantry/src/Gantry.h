...
public:
     ...
     * @param gripperOutputPin GPIO pin for gripper output (control signal) - stubbed
     * @param gripperInputPin GPIO pin for gripper input (status/feedback) - stubbed, -1 to disable
     */
    Gantry(const BergerdaServo::DriverConfig &xConfig, int gripperOutputPin, int gripperInputPin = -1);
    ...
    /**
     * @brief Control the gripper (STUB - no hardware control)
     * @param active true to close, false to open
     * @note This is a stub implementation. The gripper hardware is not connected.
     *       The output pin state is updated for simulation/testing purposes only.
     */
    void grip(bool active);
    
    /**
     * @brief Read gripper status (STUB - no hardware feedback)
     * @return bool true if gripper is active (closed), false if inactive (open)
     * @note This is a stub implementation. Returns the last set state.
     */
    bool getGripperStatus() const;
...
private:
    ...
    int gripperOutputPin_;  // Output pin for gripper control (stubbed)
    int gripperInputPin_;   // Input pin for gripper feedback (stubbed)
    int xMinPin_;
    int xMaxPin_;
